...
1) Now, we have to add Jenkins to sudoers and for that we need to make some changes in the /etc/sudoers, please add this two lines in the file.
...
1) In Build Trigger section, check "Build after other projects are built", fill "Webdev-job1" as Project to Watch and select button for "Trigger only if build is stable"
...
3) run the script ./ngrok http 8080
...
Make sure entry always ends with '/', otherwise your webhook will not sync with jenkins.
...
(to call-in the merge first you will have to configure credentials, refer this link )
...
5) Now configure Post build actions to Merge Branch 'dev' with 'master' branch in github, refer screenshot
https://github.com/VarunSinghRai/webprod/blob/master/images/Post%20build%20actions.JPG
6) Click save and apply

Automate the deployment of 'webprod' and 'webdev' webservers, by updating 'Dev' and 'Master' Branch

user curl to check your hosted docker containers 
1) curl http://hostipaddress:8081
2) curl http://hostipaddress:8082

Observe and once results are satisfactory, manually run the build of Jenkins third job "WebDev-Merge" to call the merge of 'dev' branch onto the 'master' branch.
